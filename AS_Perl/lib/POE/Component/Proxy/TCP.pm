     ...
     InlineStates       => { send => sub {
			       my ( $heap, $message ) = @_[ HEAP, ARG0 ];
                               if ($heap->{client}) {
                                  ...
                                  dbprint(5, "client is removed, so ignore sending the client's data: $message from server");
                               }
			     } },
  ...
  $heap->{proxy_client} = 
	...
	( Alias => $proxy_client_session_alias,
	  ...
	  ServerError => sub  {
	    my ($syscall_name, $error_number, $error_string) = @_[ARG0, ARG1, ARG2];
            if ($error_number) {
                dbprint(1, "ServerError ($syscall_name error $error_number: $error_string) from ORIG_SERVER on $self->{orig_port} :",
	      "$self->{orig_address}");
            }
            else {
                dbprint(1, "ServerError (error $error_number: disconnected) from ORIG_SERVER on $self->{orig_port} :",
	      "$self->{orig_address}");
                #$_[KERNEL]->post($session_id, "shutdown");
            }
